...
local function requireNew(modname)
    ...
    if love then DoesFileExist = love.filesystem.exists end

    for r in package.path:gmatch('[^;]+') do
        local mn = modname:gsub("[%.%\\]", "/")
        local fileName = r:gsub('%?', mn):gsub('^%.[/\\]', '')
        if DoesFileExist(fileName) then
            if fileCache[fileName] then break end
            local func, msg = loadfileNew(fileName)
            assert(func, msg)
